# Agent Guidelines
 ...
 - **Testing**: After making changes run the Jest test suite with `pnpm exec jest`. The project currently ships without Jest so the command fails with `Command "jest" not found`.
